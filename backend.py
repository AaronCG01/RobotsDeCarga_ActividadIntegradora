...
import uuid
from robotsDeCarga import StockRoom, Robot, Box, Stack2, Stack3, Stack4, Stack5
...
def queryState(id):
    ...
    for agent in model.schedule.agents:
        if isinstance(agent, Robot):
            content["robots"].append({"id": agent.unique_id, "lifting": agent.lifting, "x": agent.pos[0], "z": agent.pos[1]})
        elif isinstance(agent, Box):
            content["boxes"].append({"id": agent.unique_id, "active": agent.active, "x": agent.pos[0], "z": agent.pos[1]})
        elif isinstance(agent, Stack2):
            content["stacks2"].append({"id": agent.unique_id, "active": agent.active, "x": agent.pos[0], "z": agent.pos[1]})
        elif isinstance(agent, Stack3):
            content["stacks3"].append({"id": agent.unique_id, "active": agent.active, "x": agent.pos[0], "z": agent.pos[1]})
        elif isinstance(agent, Stack4):
            content["stacks4"].append({"id": agent.unique_id, "active": agent.active, "x": agent.pos[0], "z": agent.pos[1]})
        else:
            content["stacks5"].append({"id": agent.unique_id, "active": agent.active, "x": agent.pos[0], "z": agent.pos[1]})
